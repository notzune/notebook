function composition (also known as compound functions) is an operation ( $\circ$ ) that takes two functions $f$ and $g$ and produces a function $h=g\circ f$ such that $h(x)=g(f(x))$.
